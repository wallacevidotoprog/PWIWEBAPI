	...
	public class eListCollection
	{
		public eListCollection(string elFile)
		{
			...
			catch (Exception ex)
			{
				...
				Loggers.LogWriteLog(TypeLog.WARNING, TypeActionLog.EXECUTE, TypePostionLog.ERROR, "eListCollection", "CTO", ex.Message);
			}
		...
		public eList[] Load(string elFile)
		{
			...
			string[] configFiles = Directory.GetFiles(AppDomain.CurrentDomain.BaseDirectory + "\\configs", "PW_*_v" + Version + ".cfg");
					...
					if (l == ConversationListIndex)
					{
						...
						else
						{
							...
							if (Li[l].elementTypes[0].Contains("AUTO"))
							{
								...
								var t1 = (Encoding.GetEncoding("GBK"));
								var t2 = t1.GetBytes("facedata\\");
									...
									try
									{
										...
										if (a == "ID")
										{
											...
											if (l == 0)
											{
												...
												{
													Loggers.LogWriteLog(TypeLog.WARNING, TypeActionLog.EXECUTE, TypePostionLog.ERROR, "eListCollection", "Load", $"Error: Found duplicate Addon id: {idtest}");
													addonIndex[idtest] = e;
												}
											}
										}
			...
			{
				Loggers.LogWriteLog(TypeLog.WARNING, TypeActionLog.EXECUTE, TypePostionLog.ERROR, "eListCollection", "Load", $"No corressponding configuration file found! Version: {Version} Pattern: configs/PW_*_v/{Version}.cfg");
			}
		...
		public void Export(string RulesFile, string TargetFile)
		{
			...
			{
				Loggers.LogWriteLog(TypeLog.WARNING, TypeActionLog.EXECUTE, TypePostionLog.ERROR, "eListCollection", "Export", $"Rules file is missing parameter. SETVERSION: Export Failed");
				bw.Close();
				fs.Close();
				return;
			}
